...
def test_hello_char() -> None:
    assert run_forth_example("hello_char.fs") == "A"


def test_cat() -> None:
    assert run_forth_example("cat.fs", input_text="abc\n") == "abc\n"


def test_double_precision() -> None:
    assert run_forth_example("double_precision.fs") == "hi=2 lo=0\n"


def test_print_int() -> None:
    assert run_forth_example("print_int.fs", limit=300000) == "0\n123\n-1\n906609"


@pytest.mark.slow
def test_prob1() -> None:
    assert run_forth_example("prob1.fs", limit=100000000) == "906609\n"
